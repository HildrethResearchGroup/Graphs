...
class LineNumberRulerView: NSRulerView {
	...
	override func drawHashMarksAndLabels(in rect: NSRect) {
		...
		if let context = NSGraphicsContext.current?.cgContext {
			...
			// Glyphs
			var dash: unichar = ("-" as NSString).character(at: 0)
			var wrappedMarkGlyph = CGGlyph(kCGGlyphMax)
			CTFontGetGlyphsForCharacters(ctFont, &dash, &wrappedMarkGlyph, 1)
			
			// [CGGlyph][10]
			var digitGlyphs = Array(repeating: CGGlyph(kCGGlyphMax), count: 10)
			...
			lineNum += regex.numberOfMatches(in: text, options: NSRegularExpression.MatchingOptions.init(rawValue: 0), range: NSMakeRange(0, characterCount))
			
			
			
			var glyphIndex = glyphCount
